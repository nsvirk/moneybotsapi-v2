...
func SetupRoutes(e *echo.Echo, cfg *config.Config, db *gorm.DB, redisClient *redis.Client) {
	...
	indexGroup.GET("", indexHandler.GetAllIndicesNames)
	indexGroup.GET("/:exchange", indexHandler.GetIndices)
	indexGroup.GET("/:exchange/names", indexHandler.GetIndicesNames)
	indexGroup.GET("/:exchange/:index/tokens", indexHandler.GetIndexTokens)
	indexGroup.GET("/:exchange/:index/symbols", indexHandler.GetIndexSymbols)
