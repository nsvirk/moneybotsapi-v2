...
func (cs *CronService) Start() {
	...
	cs.addStartupJob("API Indices UPDATE job", cs.apiIndicesUpdateJob, 9*time.Second)
	...
	cs.logger.Info("Initializing CronService", map[string]interface{}{
		"scheduled_jobs": len(cs.c.Entries()),
	})
...
func (cs *CronService) apiInstrumentsUpdateJob() {
	...
	if err != nil {
		cs.logger.Error("API Instruments UPDATE job failed", map[string]interface{}{
			"error": err.Error(),
		})
		zaplogger.Info("")
		zaplogger.Error("API Instruments UPDATE job failed")
		zaplogger.Error("  * error    : " + err.Error())
		zaplogger.Info("")
		return
	}

	cs.logger.Info("API Instruments UPDATE job successful", map[string]interface{}{
		"total_inserted": totalInserted,
	})
	zaplogger.Info("")
	zaplogger.Info("API Instruments UPDATE job successful")
...
func (cs *CronService) apiIndicesUpdateJob() {
	...
	if err != nil {
		cs.logger.Error("API Indices UPDATE job failed", map[string]interface{}{
			"error": err.Error(),
		})
		zaplogger.Info("")
		zaplogger.Error("API Indices UPDATE job failed")
		zaplogger.Error("  * error    : " + err.Error())
		zaplogger.Info("")
		return
	}

	cs.logger.Info("API Indices UPDATE job successful", map[string]interface{}{
		"total_inserted": totalInserted,
	})
	zaplogger.Info("")
	zaplogger.Info("API Indices UPDATE job successful")
